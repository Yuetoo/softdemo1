    ...
    <div class="register-container">
      <el-form ref="Form"   :model="user"  :rules="rules" label-position="left" label-width="0px"
        ...
        <div class="info" v-show="active==0">
          <el-button style="margin-top: 12px;"  @click="correctQues" >下一步</el-button>
        </div>
        <div class="info" v-show="active==1">
          <h4  class="register-title">密保问题：</h4>
                     {{this.question}}
        ...
        <div class="info" v-if="active==3"  >
          <h3 class="title">完成修改</h3>
          <router-link :to="{path: '/login'}">  <el-button style="margin-top: 12px;"  @click="correct(user)" >完成修改</el-button></router-link>
        </div>
      </el-form>
    </div>
    <div class="footer">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            correctQues(){
                let that = this;
                axios.get("http://x238742m66.wicp.vip/getQuestion").then(function(res){
                    if(res.data.msg.match(/ok/g)){
            ...
            correct(formdata){
                ...
                axios.post('http://x238742m66.wicp.vip/changePassword',form).then(function (res) {
                    if(res.data.msg.match(/ok/g)){
                        that.next();
                    }else{
                        that.$message.error("修改失败 ∑(っ°Д°;)っ")
                    }
                }).catch(function (erro) {
